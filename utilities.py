...
def caesar_cipher(text, shift, encode=False):
    """
    Encode or decode a text using a given shift value.

    :param text: A string to be encrypted or decrypted.
    :param encode: A boolean that indicates if the ciphertext should be encoded or decoded.
    :param shift: An integer that indicates the shift of the ciphertext.
    :precondition: Text is not empty.
    :precondition: Shift is an integer.
    :precondition: Encode is a boolean.
    :postcondition: Encode or decode a text accurately.
    :return: Encoded or decoded text.

    >>> caesar_cipher("Zhvipzg", 21, False)
    'Emanuel'
    >>> caesar_cipher("Hello world!", 92, True)
    'Vszzc kcfzr!'
    >>> caesar_cipher("Az", -2, True)
    'Yx'
    """
    ...
    for letter in text:
        if letter.isalpha():
            if letter.isupper():
                set_to_zero = 65  #To shift the ASCII values to 0-26 (For simpler calculation)
            else:
                set_to_zero = 97
            if encode:
                output += chr((ord(letter) - set_to_zero + shift) % 26 + set_to_zero)
            else:
                output += chr((ord(letter) - set_to_zero - shift) % 26 + set_to_zero)
        else:
            output += letter
